...
def test_default_header_hash_and_source_are_exact_and_absolute_output_honored(
    ...
    expected_header_line = (
        ...
        f"BodyHash<<sha256.hex[:16]:{expected_hash}>> — do not hand-edit\n"
    )
...
def test_declared_truncation_length_is_honored(
    ...
    # The token declares its own truncation (BodyHash<<algorithm.hex[:N]:...>>),
    # and verification honors the declared N rather than assuming evergen's
    # default of 16 — the truncation is part of the self-describing format.
    monkeypatch.chdir(tmp_path)
    body = "X = 1\n"
...
def test_header_without_source_placeholder_works(
    ...
    assert content.splitlines()[0] == f"// BodyHash<<sha256.hex[:16]:{expected_hash}>>"
