...
def b32_hash(body: str, length: int = 8) -> str:
...
def test_body_containing_its_own_token_round_trips(
    ...
    (tmp_path / "evil.eg.py").write_text(
        "def gen():\n"
        "    return '# BodyHash<<sha256.hex[:16]:deadbeefdeadbeef>>\\nreal_code = 1\\n'\n",
        encoding="utf-8",
    )
...
def test_default_header_hash_and_source_are_exact_and_absolute_output_honored(
    ...
    expected_hash = b32_hash(body)
    expected_header_line = (
        f"# @generated by evergen from {expected_source} — "
        f"BodyHash<<sha256.b32[:8]:{expected_hash}>> — do not hand-edit\n"
    )
...
def test_bodyhash_scan_window_and_hash_domain(
    ...
    header_line = f"# BodyHash<<sha256.hex[:16]:{stored_hash}>>\n"
...
def test_declared_truncation_length_is_honored(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    # The token declares its own truncation and encoding
    # (BodyHash<<algorithm.encoding[:N]:...>>), and verification honors the
    # declared values rather than assuming evergen's defaults — this hex token
    # also proves the non-default "hex" encoding stays readable.
    monkeypatch.chdir(tmp_path)
    body = "X = 1\n"
    write_gen(tmp_path / "x.eg.py", body)
    eight = hashlib.sha256(body.encode("utf-8")).hexdigest()[:8]
    (tmp_path / "x.py").write_text(
        f"# BodyHash<<sha256.hex[:8]:{eight}>>\n{body}", encoding="utf-8"
    )

    code, out, err = run_evergen(capsys, "--check", "--output", "{}.py", "x.eg.py")

    assert code == 0, err
    assert "OK x.py <- x.eg.py" in out
...
def test_unknown_hash_algorithm_in_header_fails_loudly(
    ...
    (tmp_path / "x.py").write_text(
        "# BodyHash<<blake9.hex[:16]:deadbeefdeadbeef>>\nX = 1\n", encoding="utf-8"
    )

    code, out, err = run_evergen(capsys, "--check", "--output", "{}.py", "x.eg.py")

    assert code == 1
    assert "unknown digest scheme 'blake9.hex'" in err
    assert "Traceback" not in err


def test_unknown_digest_encoding_in_header_fails_loudly(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    # Same fail-loudly contract for the encoding half of the scheme: a token
    # naming an encoding this evergen cannot render (e.g. from a newer version)
    # is an explicit ERROR, never a silent state guess.
    monkeypatch.chdir(tmp_path)
    write_gen(tmp_path / "x.eg.py", "X = 1\n")
    (tmp_path / "x.py").write_text(
        "# BodyHash<<sha256.b64[:8]:DEADBEEF>>\nX = 1\n", encoding="utf-8"
    )

    code, out, err = run_evergen(capsys, "--check", "--output", "{}.py", "x.eg.py")

    assert code == 1
    assert "unknown digest scheme 'sha256.b64'" in err
    assert "Traceback" not in err
...
def test_check_reports_dirty_without_running_generator(
    ...
    monkeypatch.chdir(tmp_path)
    (tmp_path / "dirty.py").write_text(
        "# BodyHash<<sha256.hex[:16]:deadbeefdeadbeef>>\nbody = 1\n", encoding="utf-8"
    )
...
def test_header_without_source_placeholder_works(
    ...
    assert content.splitlines()[0] == f"// BodyHash<<sha256.b32[:8]:{expected_hash}>>"
