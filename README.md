...
```python
# @generated by evergen from models.eg.py — BodyHash<<sha256.hex[:16]:8782a01371bca95d>> — do not hand-edit
class User:
    table = "users"
...
as `algorithm.hex[:N]:digest` — it reads as the pseudocode it is: the named
algorithm's hexdigest over the generated body (excluding the header line),
sliced to its first N characters. Today that is `sha256.hex[:16]:…`; the label
says "truncated" out loud rather than implying the full digest. Newlines are
normalized to `\n` before hashing, so CRLF checkouts do not create false dirty
reports.
...
**Self-describing hash token.** The token carries its algorithm *and* its
truncation — `BodyHash<<sha256.hex[:16]:…>>` — so either can change in a
future release without another header-format break. evergen writes SHA-256
(hardware-accelerated, stdlib, more than enough for tiny generated files)
truncated to 16 hex chars, and verifies with whatever algorithm and slice the
token declares; a token naming an algorithm this Python does not provide is an
explicit error, never a silent guess.
